<template>
  <q-dialog :model-value="showDialog" @update:model-value="closeDialog" persistent>
    <q-card>
      <q-card-section>
      ...
      <q-card-section>
        <div class="row">
          ...
          >
            ...
            >
              <q-img :src="wallet.logo.src" :alt="wallet.logo.alt" style="width: 48px; height: 48px;" class="q-mr-sm" />
              <div class="column justify-center">
                <div class="text-subtitle1">{{ wallet.title }}</div>
                <div v-if="!wallet.installed" class="text-caption">Not installed</div>
              </div>
              <q-icon v-if="selectedWallets.find((s: string) => s === wallet.id)" name="check_circle" color="primary" class="q-ml-auto" />
            </q-card>
          </div>
        </div>
      </q-card-section>
...
const EVM_WALLETS = [
  {
    ...
    detect: () => !!(window as any).ethereum && (window as any).ethereum.isMetaMask
  }
];
...
async function selectWallet(wallet: WalletStatus) {
  if (selectedWallets.value.find(w => w === wallet.id)) {
    selectedWallets.value = selectedWallets.value.filter(w => w !== wallet.id)
    return
  }
  if (!wallet.installed) return;
  (wallet.id === 'metamask') ? await connectMetaMask() : await connectSubstrateWallet(wallet);
...
async function closeDialog() {
  const accounts = [await getEvmWalletAccounts(), await getSubstrateAccounts()].flat()
