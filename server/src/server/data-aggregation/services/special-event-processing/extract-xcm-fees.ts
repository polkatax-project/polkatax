...
const extractAccountId = (multiLocation: MultiLocation) => {
  const id = multiLocation.interior?.X1?.[0]?.AccountId32?.id || multiLocation.interior?.X1?.AccountId32?.id
...
export const extractXcmFees = (
  ...
  tx: TransactionDetails,
    ...
    const getFees = (e: EventDetails) => {
        return Number(getPropertyValue("fees", e)?.[0]?.fun?.Fungible ?? getPropertyValue("fees", e)?.fun?.Fungible)
    }
    ...
    if (xcmPalletFeesPaidEv.length > 0) {
        return xcmPalletFeesPaidEv.reduce(
        (curr, e) =>
            getFees(e) + curr,
        0,
        );
    ...
    if (polkadotxcmFeesPaidEv.length > 0) {
        return polkadotxcmFeesPaidEv.reduce(
        (curr, e) =>
            getFees(e) + curr,
        0,
        );
