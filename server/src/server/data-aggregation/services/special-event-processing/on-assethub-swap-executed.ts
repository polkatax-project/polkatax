...
const extractAsset = (multiLocation: MultiLocation, context: EventHandlerContext) => {
  if (multiLocation.parents === 1 && isEqual(multiLocation.interior, { Here: "NULL" })) {
    return context.tokens.find(t => t.native)
  }
  const foreignAsset = determineForeignAsset(multiLocation, context.foreignAssets)
  if (foreignAsset) {
    return foreignAsset
  }
  if (multiLocation.interior?.X2?.[1].GeneralIndex) {
    const token = context.tokens.find((a) => a.asset_id == multiLocation.interior?.X2?.[1].GeneralIndex);
    return token
  }
}
