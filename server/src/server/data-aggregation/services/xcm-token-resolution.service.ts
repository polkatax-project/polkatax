...
const extractAsset = (property: string | string[], event: EventDetails, tokens: (Asset | ForeignAsset)[]) => {
...
export class XcmTokenResolutionService {
    ...
    async resolveTokens(chain: { domain: string, token: string}, messages: XcmTransfer[], events: SubscanEvent[]): Promise<EventEnrichedXcmTransfer[]> {
        const assets = await this.fetchAssets(chain.domain)
        const tokenDepositEvents = events.filter(e => e.event_id === 'Deposited' && e.module_id !== 'balances')
        const balancesdepositEvents = events.filter(e => e.event_id === 'Deposited' && e.module_id === 'balances')
        const eventsOfInterest: SubscanEvent[] = []
        messages.forEach(xcm => xcm.transfers.filter(t => t.destChain === chain.domain).forEach((xcm: any) => {
            xcm.events = tokenDepositEvents.filter(e => e.timestamp === xcm.timestamp).map(e => {
        ...
        const eventDetails = await this.subscanService.fetchEventDetails(chain.domain, eventsOfInterest)
        const deposits = eventDetails.map(e => {
            const asset = extractAsset('currency_id', e, assets)
            const amount = Number(getPropertyValue("amount", e)) / Math.pow(10, asset?.decimals);
            return { timestamp: e.timestamp, module_id: e.module_id, original_event_id: e.original_event_index, amount, symbol: asset?.symbol, asset_unique_id: asset?.unique_id }
        })
        messages.forEach(xcm => {
            xcm.transfers.filter(t => t.destChain === chain.domain).forEach(transfer => {

                // Match by asset id of another chain
                const byAssetId = assets.find(t => t.unique_id === transfer.asset_unique_id)
                if (byAssetId) {
                ...
                const matchingDeposits = deposits.filter(d => d.timestamp).filter(d => d.symbol === symbol || d.symbol === "xc" + symbol)
                if (matchingDeposits.length === 1 || new Set(matchingDeposits.map(d => d.asset_unique_id)).entries.length === 1) {
                ...
                if (anyDeposit.length === 1 || new Set(matchingDeposits.map(d => d.asset_unique_id)).entries.length === 1) {
                ...
                if (symbol) {
                    const bySymbol = assets.filter(t => t.symbol === symbol || t.symbol === "xc" + symbol)
                ...
                if (balancesdepositEvents.filter(d => d.timestamp).length > 0 && chain.token === symbol) {
